	...
	public void run() {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (isAsync) { // Send asynchronously
				producer.send(new ProducerRecord<Integer, String>(topic, messageNo, c.toJson()),new DemoCallBack(startTime,messageNo, c.toJson()));
			} else { // Send synchronously
				try {
					producer.send(new ProducerRecord<>(topic, messageNo, c.toJson())).get();
